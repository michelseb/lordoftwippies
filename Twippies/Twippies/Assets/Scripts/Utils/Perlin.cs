...
public static class Perlin
{
    ...
    public static float Perlin3D(Vector3 coord, int offset)
    {
        var x = coord.x + offset;
        var y = coord.y + offset;
        var z = coord.z + offset;
    ...
    public static float Fbm(Vector3 coord, int octave, float offset)
    {
        var f = 0.0f;
        var w = 0.5f;
        for (var i = 0; i < octave; i++)
        {
            f += w * Noise(coord + Vector3.one * offset);
            coord *= 2.0f;
            w *= 0.5f;
        }
        return f;
